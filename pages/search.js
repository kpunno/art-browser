...
export default function Search() {
    ...
    async function submitForm(data, e) {
        e.preventDefault();
        let queryString
            = (data.searchBy ? `${data.searchBy}=true` : "")
            + (data.geoLocation ? `&geoLocation=${data.geoLocation}` : "")
            + (data.medium ? `&medium=${data.medium}` : "")
            + (`&isOnView=${data.currentlyOnView}`)
            + (`&isHighlight=${data.highlighted}`)
            + (data.q ? `&q=${data.q}` : "");
            ...
            <Row>
                <Col>
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control required type="text" placeholder="" name="q" {...register("q")}/>
                    </Form.Group>
                </Col>
            </Row>
            <Row>
